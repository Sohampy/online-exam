...
export default function QuestionUpload() {
  ...
  async function importRows() {
    ...
    setUploading(false);
    setMessage(`Imported ${insertRows.length} questions. Skipped ${validRows.length - insertRows.length} duplicates.`);
  }
  ...
  return (
    <>
      ...
      <section className="panel upload-panel">
        ...
        {rows.length > 0 && (
          <div className="upload-actions">
            ...
            <button className="btn" type="button" onClick={importRows} disabled={uploading || invalidRows.length > 0}>{uploading ? 'Importing...' : 'Import Questions'}</button>
          </div>
        )}
      ...
      {rows.length > 0 && (
        <section className="panel upload-preview">
          <h2>Preview</h2>
