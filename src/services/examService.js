...
export async function getAttemptQuestions(attemptId) {
  const { data, error } = await supabase
    .from('exam_questions')
    .select('question_order, questions(id, question_text, option_a, option_b, option_c, option_d, chapter_id, difficulty, marks, explanation)')
...
export async function getSavedAnswers(attemptId) {
  const { data, error } = await supabase
    ...
    .select('question_id,selected_option,review_status')
    .eq('attempt_id', attemptId);
  if (error) throw error;
...
export async function saveAttemptAnswer(attemptId, questionId, selectedOption, reviewStatus = 'answered') {
...
export async function submitExam(attemptId, answers, reviewStatuses = {}) {
  const { data: attempt, error: attError } = await supabase.from('student_attempts').select('*, exams(*)').eq('id', attemptId).single();
  ...
  const answerRows = questionIds.map(questionId => {
    ...
    return {
      ...
      review_status: reviewStatuses[q.id] || (selected ? 'answered' : 'not_answered')
    };
  });
  ...
  const totalMarks = Number(attempt.exams?.total_marks || questions.reduce((sum, q) => sum + Number(q.marks || 1), 0));
  const percentage = totalMarks ? Number(((totalScore / totalMarks) * 100).toFixed(2)) : accuracy;
...
export async function getQuestionWisePerformance(attemptId) {
  ...
  const chapterIds = [...new Set(questions.map(question => question.chapter_id).filter(Boolean))];
  ...
  return questions.map(question => {
    ...
    return {
      ...
      chapter_name: chapterMap[question.chapter_id] || 'Unknown',
